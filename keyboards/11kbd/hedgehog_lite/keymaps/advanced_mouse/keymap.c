...
// State keeper for awake movement signal
volatile bool keep_awake = false;
deferred_token automove_token = INVALID_DEFERRED_TOKEN;
...
const rgblight_segment_t PROGMEM my_wake_layer[] = RGBLIGHT_LAYER_SEGMENTS(
    {0, 15, RGB_PINK}
);
...
// Deferred callback and start-stop
uint32_t my_mouse_keep_awake_cb(uint32_t trigger_time, void *cb_arg) {
    for (int i=0; i<21; i++) {
...
void start_automove(void){
    automove_token = defer_exec(100, my_mouse_keep_awake_cb, NULL);
};
void stop_automove(void){
    cancel_deferred_exec(automove_token);
    automove_token = INVALID_DEFERRED_TOKEN;
};
...
void leader_end_user(void) {
    if (leader_sequence_one_key(QK_LEAD)) {
        ...
        rgblight_set_layer_state(4, keep_awake);
        if (keep_awake) {
            start_automove();
        }
        else {
